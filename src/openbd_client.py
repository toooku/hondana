...
class OpenBDClient:
        ...
        for attempt in range(cls.MAX_RETRIES):
            try:
                response = requests.get(
                    cls.BASE_URL, params={"isbn": isbn}, timeout=cls.TIMEOUT
                )
                ...
                raise cls.NetworkError(
                    f"Request timeout after {cls.MAX_RETRIES} attempts"
                )
                ...
                if attempt < cls.MAX_RETRIES - 1:
                    ...
                    continue
                raise cls.NetworkError(
                    f"Connection error after {cls.MAX_RETRIES} attempts"
                )
    ...
    def _clean_author_name(author_name: str) -> str:
        ...
        for part in space_parts:
            ...
            comma_parts = [
                subpart.strip() for subpart in part.split(",") if subpart.strip()
            ]
                                            ...
                                            if isinstance(version, dict):
                                                ...
                                                if resource_link:
                                                    ...
                                                    if (
                                                        isbn
                                                        and "-" in isbn
                                                        and isbn in resource_link
                                                    ):
                                                        resource_link = (
                                                            resource_link.replace(
                                                                isbn, isbn_clean
                                                            )
                                                        )
                                                    if OpenBDClient._verify_cover_url(
                                                        resource_link
                                                    ):
            ...
            if isinstance(hanmoto, dict):
                ...
                for key, value in hanmoto.items():
                    if (
                        ...
                        and ("jpg" in value.lower() or "png" in value.lower())
                        and "http" in value.lower()
                    ):
